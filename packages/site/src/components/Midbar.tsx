...
const Midbar = ({
  ...
  const temp =
    rooms.length > 0 ? rooms.find((room: any) => roomId == room.id) : null;
  ...
  const getUser = async () => {
    try {
      ...
      if (user != resUser.data.address) setUser(resUser.data.address);
    } catch (e) {
  ...
  useEffect(() => {
    setFormData((prev): any => ({
      ...prev,
      amountArray: membersArray.map((member): any => ({
        id: member,
        value: 0,
      })),
    }));
  ...
  const split = async () => {
    ...
    const data =
      value == 1
        ? {
            name: formData.name,
            date: new Date(),
            involved: listtobesent.map((member: any) => {
              return {
                user: member,
                amount:
                  payer == member
                    ? -1 *
                      (parseFloat(formData.amount) -
                        parseFloat(formData.amount) / listtobesent.length)
                    : parseFloat(formData.amount) / listtobesent.length,
              };
            }),
          }
        : {
            name: formData.name,
            date: new Date(),
            involved: formData.amountArray.map((item: any) => {
              return {
                user: item.id,
                amount:
                  payer == item.id
                    ? -1 * parseFloat(formData.amount) + item.value
                    : item.value,
              };
            }),
          };
    ...
    try {
      const res = await axios.post(
        `http://localhost:5000/split/${roomId}`,
        data,
        {
          headers: {
            'Content-Type': 'application/json',
  ...
  const addMember = async () => {
    ...
    try {
      const res = await axios.post('http://localhost:5000/group/add', data, {
        headers: {
          'Content-Type': 'application/json',
          Authorization: `Bearer ${access}`,
        },
  ...
  return (
    <div className="Midbar">
        ...
        >
          <Box sx={style}>
            ...
            >
              ...
              >
                ...
                <MenuItem value={30}>Thirty</MenuItem> */}
                  ...
                  </div>
                  {
                    ...
                    >
                      <div style={{ width: '40%' }}>Includes :</div>
                      <Grid
                        container
                        spacing={0.5}
                        columns={1}
                                ...
                                onChange={(event: any) => {
                                  ...
                                  if (event.target.checked) {
                                    ...
                                    if (listtobesent.length === 1) {
                                      ...
                                      const dummylist = listtobesent.filter(
                                        (items: any) => {
                                          return items != member;
                                        },
                                      );
              ...
              >
                {((listtobesent.length > 0 && value == 1) ||
                  (value == 0 && totalAmount == formData.amount)) &&
                payer &&
                formData.amount !== '' &&
                formData.name !== '' ? (
                  <Button
      ...
      {sidebarBool ? (
        ...
        >
          <div
            className="Midbar__simple"
            style={{ marginBottom: 0, paddingBottom: 0 }}
          >
            {simplified &&
              simplified != undefined &&
              Object.keys(simplified).map((item: any, index: number) => {
                console.log(
                  'Item',
                  item,
                  'MAps',
                  Object.keys(simplified[item]),
                  drop[item],
                );
                return (
                  <>
                    ...
                    <div style={{ display: drop[item] ? 'block' : 'none' }}>
                      {simplified[item] != undefined &&
                        Object.keys(simplified[item]).map(
                            ...
                            return (
                              ...
                              >
                                ...
                                <Errorboundary>
                                  {!isLoading ? (
                                    <MidbarSimple
                                      amount={simplified[item][subItem]}
                                      first={item}
                                      contacts={contacts}
                                      second={subItem}
                                      user={user}
