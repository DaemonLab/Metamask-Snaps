...
public class SimplifyDebts {
  ...
  public static void main(String[] args) {
    ...
    int n = values[0];
    Dinics solver = new Dinics(n);
    //  Adding edges to the graph
    for(int i = 1; i< args.length; i+=3) {
			solver.addEdge(values[i], values[i+1], values[i+2]);
		}
    createGraphForDebts(n, solver);
  }


  private static void createGraphForDebts(int n, Dinics solver) {
    // System.out.println();
...
abstract class NetworkFlowSolverBase {
   ...
   * @param n - The number of nodes in the graph including source and sink nodes.
   */
  public NetworkFlowSolverBase(int n) {
    this.n = n;
    initializeGraph();
  ...
  public void printEdges() {
    String out = "";
    for(Edge edge : edges) {
      out += String.format("%s %s %s ", edge.from, edge.to, edge.capacity);
    }
